...
var
  FrmTransferFirma: TFrmTransferFirma;
...
procedure TFrmTransferFirma.BtnKaydetClick(Sender: TObject);
var TmpSql:string;
begin


if trim(TxtFirmaAdi.Text)='' then begin
MsgUyari('Lütfen Transfer Firma Adýný Giriniz ...');
exit;
end;
...
Qry1.Close;
Qry1.SQL.Clear;
Qry1.SQL.Add('Select id from TransferFirma where Adi=:Adi  ');
Qry1.Parameters.ParamByName('Adi').Value     := Trim(TxtFirmaAdi.Text);
Qry1.Prepared;
Qry1.Open;

if Qry1.RecordCount<>0 then begin
MsgUyari('Kayýtlý Transfer Firma, Lütfen Deðiþtiriniz  ...');
exit;
end;


TmpSql := 'insert into TransferFirma (Adi,Tel,Adres,Email1,Email2,Fax,Msn,Notlar) ' +
' Values' +
...
procedure TFrmTransferFirma.BtnSilClick(Sender: TObject);
var TmpSql:String;
begin

if MsgOnay('Kaydý Silmek Ýstiyormusunuz ?') = False Then exit;

TmpSql:= 'Delete From TransferFirma  where id=:id ';
...
procedure TFrmTransferFirma.BtnGuncelleClick(Sender: TObject);
var TmpSql:string;
begin
...
Qry1.Close;
Qry1.SQL.Clear;
Qry1.SQL.Add('Select id from TransferFirma where  id<>:id and Adi=:Adi  ');
Qry1.Parameters.ParamByName('Adi').Value := Trim(TxtFirmaAdi.Text);
Qry1.Parameters.ParamByName('id').Value  := TxtFirmaAdi.Tag;
Qry1.Prepared;
Qry1.Open;
...
TmpSql := 'Update TransferFirma set '+
'Adi=:Adi,' +
'Tel=:Tel,' +
'Adres=:Adres, '+
'Email1=:Email1,'+
...
procedure TFrmTransferFirma.BtnTemizleClick(Sender: TObject);
begin
TxtFirmaAdi.Clear;
TxtTelefon.Clear;
TxtAdres.Clear;
...
procedure TFrmTransferFirma.CreateParams(var Params: TCreateParams);
begin
...
procedure TFrmTransferFirma.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Action := caFree;
FrmTransferFirma:= nil;
end;
